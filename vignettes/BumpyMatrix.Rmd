...
package: BumpyMatrix
date: "Revised: November 25, 2020"
output:
  BiocStyle::html_document:
    toc_float: yes
vignette: >
  %\VignetteIndexEntry{BumpyMatrix}
...
A variety of `BumpyMatrix` subclasses are implemented but the most interesting is probably the `BumpyDFrameMatrix`.
This is an S4 matrix class where each entry is a `DFrame` object, i.e., Bioconductor's wrapper around the `data.frame`.
...
We then use the `splitToBumpyMatrix()` utility to easily create our `BumpyDFrameMatrix` based on the variables on the x- and y-axes.
Here, each row is a gene, each column is a cell, and each entry holds all coordinates for that gene/cell combination.

```{r}
library(BumpyMatrix)
mat <- splitToBumpyMatrix(df[,c("x", "y")], row=df$gene, column=df$cell)
mat
mat[1,1][[1]]
```

We can also set `sparse=TRUE` to use a more efficient sparse representation, which avoids explicit storage of empty `DFrame`s.
...
```{r}
smat <- splitToBumpyMatrix(df[1:100,c("x", "y")], row=df$gene[1:100], column=df$cell[1:100])
smat
```
...
If the returned submatrix has a dimension of length 1 and `drop=TRUE`, the underlying `CompressedList` of values (in this case, the list of `DFrame`s) is returned.
...
For `BumpyDFrameMatrix` objects, we have an additional third index that allows us to easily extract an individual column of each `DFrame` into a new `BumpyMatrix`.
...
When we do `x[i]`, we effectively loop over the corresponding entries of `x` and `i`, using the latter to subset the `DFrame` in the former.
This produces a new `BumpyDFrameMatrix` containing, in this case, only the observations with positive x- and y-coordinates.

For `BumpyDFrameMatrix` objects, subsetting to a single field in the third dimension will automatically drop to the type of the underlying column of the `DFrame`.
This can be stopped with `drop=FALSE` to preserve the `BumpyDFrameMatrix` output:
